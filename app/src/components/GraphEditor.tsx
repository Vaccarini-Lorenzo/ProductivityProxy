import type { GraphNode, ModeConfig, NodeParams } from "../models/config/types";
...
export function GraphEditor(props: Props) {
  ...
  return (
    <section className="panel graph-panel" aria-labelledby="graph-heading">
      ...
      <div className="node-toolbar" aria-label="Add graph node">
        {BUILT_INS.map((type) => (
          <button key={type} type="button" onClick={() => props.onAddNode(type)}>
            Add {type}
          </button>
        ))}
      </div>
      ...
      <div className="graph-canvas" role="img" aria-label={`Graph for ${props.mode.name}`}>
        <svg className="graph-edges" aria-hidden="true">
          {props.mode.graph.edges.map((edge, index) => (
            <GraphEdgeLine key={`${edge.from}-${edge.to}-${index}`} from={findNode(props.mode, edge.from)} to={findNode(props.mode, edge.to)} />
          ))}
        </svg>
      ...
      <div className="editor-grid">
        <div>
          ...
          <button type="button" onClick={props.onApplyParams} disabled={!selectedNode}>
            Apply params
          </button>
        </div>
        <div>
...
function GraphEdgeLine({ from, to }: { from?: GraphNode; to?: GraphNode }) {
  if (!from || !to) return null;
  const fromPosition = from.position ?? { x: 80, y: 120 };
  const toPosition = to.position ?? { x: 220, y: 120 };
  return <line x1={fromPosition.x + 80} y1={fromPosition.y + 24} x2={toPosition.x} y2={toPosition.y + 24} />;
}
