...
- System proxy state is captured in memory **and** persisted to `system_proxy_snapshot.json`, so it is restored automatically on the next start/stop/status and on app exit; manual recovery is a fallback.
